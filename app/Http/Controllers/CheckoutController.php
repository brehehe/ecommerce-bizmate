        ...
        DB::transaction(function () use (
            ...
            if ($paymentMethod->type === 'gateway') {
                ...
                if ($methodName === 'qris (komerce)') {
                    ...
                    if ($isMidtrans) {
                        $midtransPaymentTypeKey = $request->input('midtrans_payment_type_key');

                        // Use Core API if a specific payment type key was provided
                        if ($midtransPaymentTypeKey) {
                            try {
                                ...
                                if ($result['success']) {
                                    ...
                                    $errorMsg = $result['error'] ?? 'Gagal menghubungi Midtrans Core API.';
                                    Log::error('Midtrans Core API Failed: '.$errorMsg);
                                    $paymentData['gateway_status'] = 'FAILED_API';
                                    $paymentData['gateway_response'] = json_encode(['error' => $errorMsg]);
                                }
                            } catch (\Exception $e) {
                            ...
                            // Fallback: use Midtrans Snap (legacy)
                            try {
