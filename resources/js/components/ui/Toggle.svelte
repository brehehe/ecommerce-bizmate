...
<button 
    type="button" 
    class="w-full flex items-center justify-between gap-6 p-3 rounded-2xl border border-slate-100 transition text-left {disabled ? 'opacity-50 cursor-not-allowed bg-slate-50' : 'hover:bg-slate-50 cursor-pointer'}" 
...
>
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            {/if}
            {#if description}
                <span class="text-[10px] text-slate-400 font-semibold block">{description}</span>
            {/if}
        </div>
    </div>
    <label class="relative inline-flex items-center cursor-pointer pointer-events-none">
        ...
        <div class="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all {disabled ? 'peer-checked:bg-slate-300' : 'peer-checked:bg-brand-blueRoyal'}"></div>
    </label>
</button>
