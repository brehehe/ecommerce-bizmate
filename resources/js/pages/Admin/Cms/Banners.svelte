<script lang="ts">
    ...
    import ImageEditorModal from '@/components/ImageEditorModal.svelte';
    ...
    function openImageEditor(
        src: string,
        type: 'hero' | 'hero_mobile' | 'side' | 'middle_wide' | 'popup',
        index: number | null = null,
    ) {
        if (!src) return;
        editorImageSrc = src;
        editorBannerType = type;
    ...
    function handleEditorSave(file: File, previewUrl: string) {
        if (editorBannerType === 'hero' && editorTargetIndex !== null) {
            ...
            form.side_files[editorTargetIndex] = file;
            form.side_banners[editorTargetIndex].image = previewUrl;
        } else if (editorBannerType === 'middle_wide') {
            form.middle_wide_file = file;
            form.middle_wide_banner.image = previewUrl;
        } else if (editorBannerType === 'popup') {
            form.popup_file = file;
            form.popup_banner.image = previewUrl;
        }
        showToast(
            'Gambar banner berhasil diperbarui dengan hasil editan!',
            'success',
        );
    }
    ...
    const form = useForm({
        ...
        middle_wide_banner: middleWideBanner
            ? {
                  ...
                  is_active:
                      middleWideBanner.is_active !== undefined
                          ? !!middleWideBanner.is_active
                          : true,
              }
            : {
        ...
        hero_mobile_files: {} as Record<number, File>,
    ...
    function addHeroBanner() {
        form.hero_banners = [
            ...form.hero_banners,
            { image: '', mobile_image: '', alt: 'Banner Baru', link: '#' },
        ];
    }
    function removeHeroBanner(index: number) {
        ...
        const newMobileFiles: Record<number, File> = {};
    ...
    function validateImageOrientation(
        ...
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                ...
                if (expected === 'landscape' && img.width <= img.height)
                    resolve(false);
                else if (expected === 'portrait' && img.height <= img.width)
                    resolve(false);
                else resolve(true);
            };
    ...
    function validateImageMinHeight(
        file: File,
        minHeight: number,
    ): Promise<boolean> {
        return new Promise((resolve) => {
    ...
    async function handleHeroMobileFileChange(index: number, e: Event) {
        const target = e.target as HTMLInputElement;
        const file = target.files?.[0];
        const key = `hero_mobile_${index}`;
        delete uploadErrors[key];
        if (!file) return;
        if (file.size > 10 * 1024 * 1024) {
            uploadErrors[key] = 'Maks. 10MB.';
        ...
        const previewUrl = URL.createObjectURL(file);
        form.hero_mobile_files[index] = file;
        form.hero_banners[index].mobile_image = previewUrl;
        target.value = '';
...
<AdminLayout>
    <div class="flex-grow flex flex-col min-h-screen">
        <main class="flex-grow p-4 sm:p-8 w-full max-w-full mx-auto space-y-6">
            ...
            >
                <div>
                    <div class="flex items-center gap-2.5 mb-1">
                        <div
                            class="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-sm shrink-0"
                        >
                            <i class="ti ti-photo-filled text-white text-base"
                            ></i>
                        </div>
                        <h1
                    ...
                    <p class="text-sm text-slate-500 font-medium ml-11">
                        Kelola banner promosi halaman depan & lihat pratinjau
                ...
                <button
                    ...
                    class="w-full sm:w-auto px-5 py-2.5 text-white font-bold rounded-xl text-sm transition duration-200 shadow-lg font-outfit uppercase tracking-wider disabled:opacity-50 flex items-center justify-center gap-2 shrink-0 cursor-pointer"
            ...
            <!-- Panduan Ukuran Banner Info Card (Light & Modern Theme) -->
            <div
                class="bg-gradient-to-r from-blue-50/80 via-indigo-50/80 to-purple-50/80 border border-blue-200/70 rounded-2xl p-5 shadow-xs relative overflow-hidden"
            >
                <div
                ...
                >
                    <div class="flex items-center gap-3">
                        <div
                            class="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center text-white shrink-0 shadow-md shadow-blue-500/20"
                        >
                            <i class="ti ti-ruler-measure text-xl"></i>
                        </div>
                        <div>
                            <h2
                                class="font-outfit font-black text-base sm:text-lg text-slate-800 tracking-tight flex items-center gap-2"
                            >
                                Panduan Dimensi & Ukuran Banner Ideal
                                <span
                                    class="text-[10px] font-bold bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-full uppercase tracking-wider hidden sm:inline-block"
                                    >Rekomendasi CMS</span
                                >
                            </h2>
                            <p class="text-xs text-slate-500 font-medium">
                                Gunakan rekomendasi ukuran berikut agar banner
                                selalu pas, tajam, dan tidak terpotong di layar
                                HP & Laptop.
                            </p>
                        </div>
                    </div>
                    <button
                        ...
                        class="px-3.5 py-1.5 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-xl text-xs font-bold transition flex items-center gap-1.5 shrink-0 shadow-xs cursor-pointer"
                    >
                        <i
                            class="ti {showGuide
                                ? 'ti-chevron-up'
                                : 'ti-chevron-down'} text-sm text-slate-400"
                        ></i>
                        <span>{showGuide ? 'Sembunyikan' : 'Lihat Detail'}</span
                        >
                    </button>
                </div>
                ...
                {#if showGuide}
                    ...
                    >
                        <!-- Item 1: Hero Banner -->
                        <div
                            class="bg-white border border-blue-100/80 rounded-xl p-3.5 space-y-1.5 shadow-2xs hover:border-blue-300 transition"
                        >
                            <div class="flex items-center justify-between">
                                <span
                                    class="text-[10px] font-black text-blue-600 uppercase tracking-wider"
                                    >1. Hero Slider (Utama)</span
                                >
                                <span
                                    class="text-[10px] font-bold text-blue-700 bg-blue-50 border border-blue-200/80 px-1.5 py-0.5 rounded"
                                    >Rasio 3 : 1</span
                                >
                            </div>
                            <div
                            ...
                            <!-- <div class="text-[10px] text-slate-600 pt-1.5 border-t border-slate-100 flex items-start gap-1">
                                ...
                                <span>Teks di tengah, margin 20-30px dari tepi.</span>
                            </div> -->
                        </div>
                        ...
                        >
                            ...
                            <p class="text-[11px] text-slate-500 leading-snug">
                                Alt: 1440 × 400 px / 1200 × 330 px
                            </p>
                            <!-- <div class="text-[10px] text-slate-600 pt-1.5 border-t border-slate-100 flex items-start gap-1">
                                <i class="ti ti-bulb text-amber-500 text-xs shrink-0 mt-0.5"></i>
                                <span>Promo Flash Sale & Event Tanggal Kembar.</span>
                            </div> -->
                        </div>
                        ...
                        >
                            ...
                            <p class="text-[11px] text-slate-500 leading-snug">
                                Alt: 800 × 800 px (Persegi 1:1)
                            </p>
                            <!-- <div class="text-[10px] text-slate-600 pt-1.5 border-t border-slate-100 flex items-start gap-1">
                                <i class="ti ti-bulb text-amber-500 text-xs shrink-0 mt-0.5"></i>
                                <span>Popup otomatis saat pengunjung masuk.</span>
                            </div> -->
                        </div>
                    </div>
                {/if}
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
                ...
                <div class="xl:col-span-7 space-y-5">
                    ...
                    >
                        <div
                            class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-indigo-50/30 flex items-center justify-between"
                        >
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-sm shrink-0"
                                >
                                    <i
                                        class="ti ti-slideshow text-white text-sm"
                                    ></i>
                                </div>
                                <div>
                                    <h3
                                    ...
                                    >
                                        Banner slide utama (Ukuran ideal: 1500 ×
                                        400 px · Rasio 15:4)
                                    </p>
                                </div>
                            </div>
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    {#if banner.image}
                                                        <div
                                                            class="relative rounded-xl overflow-hidden aspect-[3/1] border border-slate-200 shadow-sm bg-white group/preview"
                                                        >
                                                            <img
                                                                src={banner.image}
                                                                alt="Preview Desktop"
                                                    ...
                                                    {:else}
                                                        <label
                                                            class="rounded-xl border-2 border-dashed border-slate-300 hover:border-blue-400 bg-white aspect-[3/1] flex flex-col items-center justify-center cursor-pointer transition group/drop hover:bg-blue-50/30 py-3"
                                                        >
                                                            <input
                                                                type="file"
                                                                accept="image/*"
                    ...
                    >
                        <div
                            class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-amber-50 to-orange-50/30 flex items-center justify-between"
                        >
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center shadow-sm shrink-0"
                                >
                                    <i class="ti ti-panorama text-white text-sm"
                                    ></i>
                                </div>
                                <div>
                                    <h3
                                    ...
                                    <p
                                        class="text-[11px] text-slate-400 font-medium"
                                    >
                                        Banner memanjang tengah (Ukuran ideal:
                                        1800 × 600 px · Alt: 1440 × 400 px)
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                ...
                                >
                                    <span
                                        class="text-xs font-bold {form
                                            .middle_wide_banner.is_active
                                            ? 'text-amber-600'
                                            : 'text-slate-400'}"
                                    >
                                    ...
                                    <div class="relative">
                                        ...
                                        />
                                        <div
                                            class="w-10 h-6 bg-slate-200 rounded-full peer peer-checked:bg-amber-500 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-4 after:shadow-sm relative"
                                        ></div>
                                    </div>
                                </label>
                                <button
                    ...
                    >
                        <div
                            class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-violet-50 to-purple-50/30 flex items-center justify-between"
                        >
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-sm shrink-0"
                                >
                                    <i
                                        class="ti ti-message-2 text-white text-sm"
                                    ></i>
                                </div>
                                <div>
                                    <h3
                                    ...
                                    <p
                                        class="text-[11px] text-slate-400 font-medium"
                                    >
                                        Popup promo otomatis saat masuk beranda
                                        (Ukuran ideal: 800 × 1000 px · Rasio 4:5
                                        / 1:1)
                                    </p>
                                </div>
                            </div>
                            ...
                            >
                                <span
                                    class="text-xs font-bold {form.popup_banner
                                        .is_active
                                        ? 'text-violet-600'
                                        : 'text-slate-400'}"
                                >
                                ...
                                <div class="relative">
                                    ...
                                    />
                                    <div
                                        class="w-10 h-6 bg-slate-200 rounded-full peer peer-checked:bg-violet-500 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-4 after:shadow-sm relative"
                                    ></div>
                                </div>
                            </label>
                        </div>
                            ...
                            >
                                ...
                                >
                                    <div class="sm:col-span-4">
                                        {#if form.popup_banner.image}
                                            <div
                                                class="relative rounded-xl overflow-hidden border border-slate-200 shadow-sm bg-white group/preview max-w-[130px] mx-auto sm:mx-0 {form
                                                    .popup_banner
                                            ...
                                            <button
                                                ...
                                                class="mt-2 w-full py-1.5 px-2 bg-violet-50 hover:bg-violet-100 text-violet-700 text-xs font-bold rounded-lg border border-violet-200 transition flex items-center justify-center gap-1 shadow-2xs max-w-[130px] mx-auto sm:mx-0 cursor-pointer"
                                            >
                                                <i
                                                    class="ti ti-photo-edit text-sm"
                                                ></i>
                                        ...
                                        {:else}
                                            <label
                                                class="rounded-xl border-2 border-dashed border-slate-300 hover:border-violet-400 bg-white flex flex-col items-center justify-center cursor-pointer transition group/drop hover:bg-violet-50/30 max-w-[130px] mx-auto sm:mx-0 {form
                                                    .popup_banner
                                                    .orientation === 'landscape'
                                                    ? 'aspect-[16/9]'
                                                    : 'aspect-[4/5]'}"
                                            >
                                                ...
                                                />
                                                <i
                                                    class="ti ti-photo-up text-xl text-slate-300 group-hover/drop:text-violet-400 transition mb-1"
                                                ></i>
                                                <span
                                                    class="text-[10px] font-bold text-slate-400 group-hover/drop:text-violet-500 text-center"
                                                    >Upload Popup</span
                                                >
                                            </label>
                                        {/if}
                                        {#if uploadErrors['popup'] || form.errors['popup_file']}
                                            <div
                                                class="mt-1.5 text-[10px] font-bold text-rose-600 flex items-start gap-1 bg-rose-50 border border-rose-100 p-2 rounded-lg max-w-[130px] mx-auto sm:mx-0"
                                            >
                                                <i
                                                    class="ti ti-alert-circle text-xs shrink-0 mt-0.5"
                                                ></i>
                                    ...
                                    <div class="sm:col-span-8 space-y-3">
                                        <div>
                                            <label
                                                ...
                                                >Orientasi Gambar</label
                                            >
                                            <div class="grid grid-cols-2 gap-2">
                                                <label
                                                    class="flex items-center gap-2 p-2.5 bg-white border rounded-xl cursor-pointer hover:bg-violet-50 transition {form
                                                        .popup_banner
                                                        .orientation ===
                                                    'portrait'
                ...
                <div class="xl:col-span-5 xl:sticky xl:top-8 space-y-4">
                    ...
                    >
                        <!-- Preview header -->
                        <div
                            class="px-5 py-3.5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-slate-100/50 flex items-center justify-between"
                        >
                            <div class="flex items-center gap-2">
                                <span
                                ...
                                ></span>
                                <h3
                                    class="font-outfit font-black text-slate-700 text-xs uppercase tracking-wider"
                                >
                        ...
                        <!-- iPhone Mockup -->
                        <div
                            class="p-6 flex justify-center bg-gradient-to-b from-slate-100 to-slate-200/60"
                        >
                            <div
                                class="relative w-[360px] h-[720px] bg-slate-900 rounded-[42px] border-[10px] border-slate-900 shadow-2xl overflow-hidden ring-4 ring-slate-800 select-none"
                            >
                                <!-- Dynamic Island -->
                                <div
                                    class="absolute top-1.5 left-1/2 -translate-x-1/2 w-28 h-5.5 bg-black rounded-full z-50 flex items-center justify-center gap-1.5"
                                >
                                    <div
                                        class="w-2 h-2 rounded-full bg-slate-950/80"
                                    ></div>
                                    <div
                                ...
                                <!-- Home Bar -->
                                <div
                                    class="absolute bottom-1 left-1/2 -translate-x-1/2 w-24 h-1 bg-black/60 rounded-full z-50"
                                ></div>
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        <h4
                            class="font-bold flex items-center gap-1.5 mb-2 text-[11px] uppercase tracking-wider text-blue-600"
                        >
                            <i class="ti ti-info-circle text-sm"></i> Cara Kerja Pratinjau
                        </h4>
                        <p class="text-xs text-blue-700 leading-relaxed">
                            Pratinjau memuat halaman depan toko secara
                            interaktif. Klik
                            <strong>Simpan Perubahan</strong> untuk mempublikasikan
                            dan melihat langsung efek banner di mockup.
                        </p>
                    </div>
