<script>
    ...
    let { courier = {}, transactions = { data: [], links: [], total: 0 } } = $props();
    ...
    const activeDeliveries = $derived(
        transactions.data.filter(t => !['selesai', 'batal'].includes(t.status)).length
    );
    ...
    <div class="flex-grow flex flex-col min-h-screen">
        <main class="flex-grow p-4 sm:p-8 w-full max-w-full mx-auto space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-3xl border border-slate-200/80 p-6 flex items-center gap-4 shadow-card">
                    <div class="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center text-xl shrink-0">
                        <i class="ti ti-truck-delivery"></i>
                    </div>
                    <div>
                        <p class="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Total Tugas</p>
                        <p class="text-2xl font-black text-slate-800 font-outfit leading-tight mt-0.5">{totalDeliveries}</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl border border-slate-200/80 p-6 flex items-center gap-4 shadow-card">
                    ...
                    <div>
                        <p class="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Sedang Aktif (Hal Ini)</p>
                        <p class="text-2xl font-black text-slate-800 font-outfit leading-tight mt-0.5">{activeDeliveries}</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl border border-slate-200/80 p-6 flex items-center gap-4 shadow-card">
                    ...
                    <div>
                        <p class="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Tugas Selesai (Hal Ini)</p>
                        <p class="text-2xl font-black text-slate-800 font-outfit leading-tight mt-0.5">{completedDeliveries}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200/80 shadow-card overflow-hidden">
                <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/20">
                    <h4 class="font-outfit font-black text-sm text-slate-800 uppercase tracking-wider">
                        Daftar Pengiriman
                    </h4>
                    <span class="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-bold font-mono">
                ...
                {#if transactions.data.length === 0}
                    <div class="py-16 text-center text-slate-400 font-bold font-outfit">
                        ...
                        Kurir ini belum pernah ditugaskan mengambil atau mengirim paket.
                    </div>
                {:else}
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b border-slate-100 bg-slate-50/50 text-[10px] font-bold text-slate-400 uppercase tracking-widest font-outfit">
                                    ...
                                    <th class="py-5 px-6">Kode Booking / Resi</th>
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition duration-150">
                                        ...
                                        <td class="py-5 px-6">
                                            <div>
                                                ...
                                                <p class="text-[11px] text-slate-400 font-bold mt-0.5">{trx.user?.phone_number ?? '-'}</p>
                                            </div>
                                        </td>
                                        <td class="py-5 px-6">
                                            <div class="max-w-[200px] truncate">
                                                {#if trx.customer_address}
                                                    <p class="text-xs text-slate-600 truncate">{trx.customer_address.district_name}, {trx.customer_address.regency_name}</p>
                                                {:else}
                                                    <p class="text-xs text-slate-400 italic">Alamat tidak tersedia</p>
                                                {/if}
                                            </div>
                                        </td>
                                        <td class="py-5 px-6 font-mono text-xs space-y-1">
                                            {#if trx.booking_code}
                                                <div class="flex items-center gap-1">
                                                    <span class="text-slate-400 uppercase text-[9px] font-bold">Book:</span>
                                                    <span class="text-slate-700 font-bold">{trx.booking_code}</span>
                                                </div>
                                            {/if}
                                            {#if trx.tracking_number}
                                                <div class="flex items-center gap-1">
                                                    <span class="text-emerald-500 uppercase text-[9px] font-bold">Resi:</span>
                                                    <span class="text-emerald-700 font-bold">{trx.tracking_number}</span>
                                                </div>
                                            {/if}
                                        ...
                                        <td class="py-5 px-6">
                                            <span class="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border {statusColors[trx.status] ?? 'bg-slate-55 text-slate-600 border-slate-200/50'}">
                                                {statusLabels[trx.status] ?? trx.status}
                                            </span>
                                        </td>
                                        <td class="py-5 px-6 font-bold text-slate-800 font-outfit text-right">
                                            {fmt(trx.grand_total)}
                                        </td>
