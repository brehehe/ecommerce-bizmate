<script lang="ts">
    ...
    let { stickers = { data: [], links: [], total: 0 }, filters = {} } = $props();
    ...
    function saveSticker(e: SubmitEvent) {
        ...
        if (isEditing) {
            ...
            router.post(`/admin/master-data/stickers/${editId}`, data, {
                forceFormData: true,
                onSuccess: () => { showToast('Stiker berhasil diperbarui', 'success'); closeModal(); },
                onError: (err: any) => { if (err.error) { showToast(err.error, 'error'); } },
            });
        } else {
            router.post('/admin/master-data/stickers', data, {
                forceFormData: true,
                onSuccess: () => { showToast('Stiker berhasil ditambahkan', 'success'); closeModal(); },
                onError: (err: any) => { if (err.error) { showToast(err.error, 'error'); } },
            });
        }
    }
    ...
    function executeBulkDelete() {
        ...
        router.post(
            ...
            },
            {
                ...
                onError: (err) => {
                    const first = Object.values(err)[0] || 'Gagal menghapus stiker terpilih.';
    ...
    function executeDelete() {
        ...
        router.delete(`/admin/master-data/stickers/${itemToDelete.id}`, {
            onSuccess: () => { showToast('Stiker berhasil dihapus', 'success'); deleteModalOpen = false; itemToDelete = null; },
            onError: (err: any) => { showToast(err?.error || 'Gagal menghapus stiker', 'error'); deleteModalOpen = false; },
        });
    }

    function toggleStatus(sticker: any) {
        router.post(`/admin/master-data/stickers/${sticker.id}/toggle-active`, {}, {
            preserveScroll: true,
            onSuccess: () => { showToast(`Status stiker ${sticker.name} berhasil diubah`, 'success'); },
            onError: (err: any) => { showToast(err?.error || 'Gagal mengubah status', 'error'); },
        });
    }
</script>
            ...
            <div class="bg-white rounded-3xl border border-slate-200/80 shadow-card overflow-hidden">
                ...
                <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-slate-50/20">
                    <div class="shrink-0 flex items-center gap-3">
                        ...
                        {#if stickers.data.length > 0}
                            <label class="flex items-center gap-2 cursor-pointer text-xs font-bold text-slate-500 font-outfit uppercase select-none">
                                <input
                                    type="checkbox"
                                    checked={selectAll}
                ...
                {#if stickers.data.length === 0}
                    <div class="py-16 text-center text-slate-400 font-bold font-outfit">
                        <i class="ti ti-sticker text-5xl block mb-3 text-slate-300"></i>
                ...
                {:else}
                    ...
                    <div class="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                        {#each stickers.data as sticker (sticker.id)}
                            ...
                            <div class="group relative bg-slate-50 rounded-2xl border {isSelected ? 'border-brand-blueRoyal bg-brand-blueRoyal/5' : 'border-slate-200'} overflow-hidden flex flex-col items-center p-3 gap-2 transition hover:shadow-md {!isActive ? 'opacity-50' : ''}">
                                <!-- Checkbox -->
                                ...
                                <!-- Preview image -->
                                <div class="w-full aspect-square flex items-center justify-center rounded-xl overflow-hidden bg-white border border-slate-100">
                                    <img
                                        src={sticker.url}
                                        alt={sticker.name}
                                ...
                                <div class="w-full text-center">
                                    <p class="text-[11px] font-bold text-slate-800 truncate">{sticker.name}</p>
                                    {#if sticker.category}
                                        <p class="text-[10px] text-slate-400 truncate">{sticker.category}</p>
                                    {/if}
                                </div>

                                <!-- Status badge -->
                                <span class="px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider {isActive ? 'bg-emerald-50 text-emerald-600 border border-emerald-200/50' : 'bg-slate-100 text-slate-500'}">
                                ...
                                <div class="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <button
                                        ...
                                        class="w-7 h-7 rounded-lg bg-white border border-slate-200 {isActive ? 'hover:bg-amber-50 hover:text-amber-600' : 'hover:bg-emerald-50 hover:text-emerald-600'} text-slate-500 flex items-center justify-center shadow-sm transition"
                                        title="Toggle Status"
                                    >
...
{#if isModalOpen}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div
            ...
            tabindex="0"
        ></div>
        <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-in fade-in zoom-in duration-200">
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <h3 class="font-outfit font-black text-lg text-slate-800">
                    {isEditing ? 'Ubah Stiker' : 'Tambah Stiker Baru'}
                </h3>
                <button aria-label="Tutup" type="button" onclick={closeModal} class="p-1 text-slate-400 hover:text-slate-700 transition">
            ...
            <form onsubmit={saveSticker} class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-2">
                        Gambar Stiker {#if !isEditing}<span class="text-rose-500">*</span>{/if}
                    </label>
                    <div
                    ...
                    >
                        {#if imagePreview}
                            <img src={imagePreview} alt="Preview" class="max-h-40 object-contain p-4" />
                            <span class="text-[10px] text-slate-400 font-bold mt-1">Klik untuk ganti</span>
                        {:else}
                            <i class="ti ti-photo-up text-3xl text-slate-300 mb-2"></i>
                            <span class="text-xs font-bold text-slate-400">Klik untuk upload gambar</span>
                            <span class="text-[10px] text-slate-300 mt-1">PNG, JPG, GIF, WebP — maks 2MB</span>
                        {/if}
                    </div>
                    <input
...
{#if deleteModalOpen}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full relative z-10 shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <h4 class="font-outfit font-black text-xl text-center text-slate-800 mb-2">Hapus Stiker?</h4>
            <p class="text-sm text-center text-slate-500 font-medium mb-8">
                Stiker <strong>{itemToDelete?.name}</strong> akan terhapus secara permanen beserta file gambarnya.
            </p>
            <div class="flex items-center gap-3">
                <button onclick={() => (deleteModalOpen = false)} class="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-sm transition">
                    Batal
                </button>
                <button onclick={executeDelete} class="flex-1 py-3 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl text-sm shadow-lg shadow-red-500/30 transition">
                    Ya, Hapus
                </button>
            </div>
        </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
            ...
            <p class="text-sm text-center text-slate-555 font-medium mb-8">
                Apakah Anda yakin ingin menghapus <strong>{selectedStickers.length} stiker</strong> yang terpilih secara permanen dari sistem? Tindakan ini tidak dapat dibatalkan.
            </p>
            <div class="flex items-center gap-3">
                <button
                    onclick={() => (deleteBulkModalOpen = false)}
