<script>
    ...
    const form = useForm({
        ...
        early_access_min_level_order: p.early_access_min_level_order ?? 0,
        photos: [],
        variations: [],
        variants: [],
    ...
    let canvas2dPanX = $state(0);
    let canvas2dPanY = $state(0);
    ...
    function generateCombinations() {
        ...
        const effectiveSku = form.sku || product.sku || (form.name ? form.name.trim().toUpperCase().replace(/[^A-Z0-9\s-]/g, '').replace(/\s+/g, '-').substring(0, 20) : '');
...
<svelte:head>
    <title>Edit Produk: {product.name} — Admin</title>
</svelte:head>
...
<AdminLayout>
    <main class="w-full max-w-[1600px] mx-auto px-4 sm:px-6 py-6 space-y-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-semibold tracking-tight text-slate-900">Edit Produk</h1>
                <p class="mt-0.5 text-sm text-slate-550">Perbarui data produk mebel di katalog toko Anda</p>
            </div>
            <Link
