<script lang="ts">
    ...
    import { update as adminPromotionsUpdate } from '@/routes/admin/promotions';
    ...
    // Helper to format ISO datetime string to YYYY-MM-DD (for HTML date input)
    ...
    // Helper to format ISO datetime string to YYYY-MM-DDTHH:MM (for HTML datetime-local input)
    ...
    function getInitialFormState() {
        return {
            ...
            member_early_access_minutes: promotion.member_early_access_minutes ?? 0,
...
<AdminLayout>
    <main class="w-full max-w-[1600px] mx-auto px-4 sm:px-6 py-6 space-y-5">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-semibold tracking-tight text-slate-900 font-sans">Edit Promosi</h1>
                <p class="mt-0.5 text-sm text-slate-500 font-sans">Sesuaikan detail parameter promosi di bawah ini</p>
            </div>
        </div>
        ...
        >
            ...
            <div class="lg:col-span-2 space-y-6">
                <div
                    class="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-6"
                >
                    <h2
                        class="text-base font-semibold text-slate-900 pb-3 border-b border-slate-100 font-sans"
                    >
                ...
                {#if form.type === 'bundling_gift'}
                    ...
                    >
                        ...
                        >
                            <div>
                                <h2
                                    class="text-base font-semibold text-slate-900 font-sans"
                                >
            ...
            <div class="w-full space-y-6">
                <div
                    class="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-6 sticky top-6"
                >
                    <h2
                        class="text-base font-semibold text-slate-900 pb-3 border-b border-slate-100 font-sans"
                    >
                    ...
                    <div class="pt-4">
                        ...
                        >
                            {#if form.processing}
                                ...
                                ></i> Menyimpan...
                            {:else}
                                <i class="ti ti-device-floppy text-lg"></i> Perbarui
                                Promosi
                            {/if}
                        </button>
                    </div>
            ...
            {#if form.type !== 'bundling_gift' && form.type !== 'voucher_gratis_ongkir' && form.type !== 'promo_toko' && form.type !== 'voucher_belanja'}
                <div
                    class="lg:col-span-3 bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-6 w-full"
                >
                    <!-- Section Header -->
                    <div
                    ...
                    >
                        <span
                            class="w-5 h-5 rounded-full bg-blue-50 text-brand-blueRoyal font-bold text-xs flex items-center justify-center"
                        >
                            4
                        </span>
                        <h2
                            class="text-sm font-semibold text-slate-800 tracking-wide uppercase font-sans"
                        >
    ...
    {#if showModal}
        ...
        >
            ...
            <!-- Modal content -->
            <div
                class="relative bg-white rounded-xl w-full max-w-4xl shadow-lg flex flex-col max-h-[85vh] overflow-hidden z-10 border border-slate-100"
            >
                <!-- Close button -->
                <button
                    type="button"
                    ...
                    onclick={() => (showModal = false)}
                    class="absolute top-6 right-6 w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center hover:bg-slate-200 text-slate-500 transition"
                >
                ...
                <div class="p-6 pb-4 border-b border-slate-150">
                    <h2 class="text-base font-semibold text-slate-900 font-sans">
                        Pilih Produk Target
                    </h2>
                    <p
                        class="text-xs text-slate-500 mt-1 font-sans"
                    >
                ...
                >
                    ...
                    >
                        <table class="w-full text-left border-collapse text-xs modal-table">
                            <thead>
                                <tr
                                    class="bg-slate-50 border-b border-slate-200"
                                >
                ...
                <!-- Footer -->
                <div
                    class="p-4 border-t border-slate-150 flex justify-end gap-3 bg-slate-50"
                >
                    <button
                        type="button"
                        onclick={() => (showModal = false)}
                        class="h-9 px-4 border border-slate-200 hover:bg-slate-100 text-slate-700 text-xs font-semibold rounded-lg transition duration-200"
                    >
                        Batal
                    </button>
                    <button
                        ...
                        class="h-9 px-4 text-white text-xs font-semibold rounded-lg transition duration-200 shadow-sm"
...
<style>
    @media (max-width: 640px) {
        ...
        .modal-table :global(td) {
            display: block !important;
            padding: 0 !important;
            border: none !important;
        }
        /* Checkbox cell */
        .modal-table :global(td:nth-child(1)) {
            width: auto !important;
            flex-shrink: 0;
        }
        /* Product info cell */
        .modal-table :global(td:nth-child(2)) {
            flex-grow: 1;
            min-width: 0;
        }
        /* Hide category cell on mobile in modal to save space */
        .modal-table :global(td:nth-child(3)) {
            display: none !important;
        }
        /* Price and stock cells */
        .modal-table :global(td:nth-child(4)),
        .modal-table :global(td:nth-child(5)) {
            width: auto !important;
