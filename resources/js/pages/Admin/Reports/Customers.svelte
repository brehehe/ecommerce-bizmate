<script lang="ts">
    ...
    let transactionsData = $state<any>({ data: [], links: [], current_page: 1, last_page: 1, total: 0 });
    let isLoadingTransactions = $state(false);

    async function fetchTransactions(customerId: number, page: number = 1) {
        isLoadingTransactions = true;
        try {
            const response = await fetch(`/admin/reports/customers/${customerId}/transactions?page=${page}`, {
                headers: {
    ...
    function openCustomerDetail(customer: any) {
        ...
        showModal = true;
        transactionsData = { data: [], links: [], current_page: 1, last_page: 1, total: 0 };
    ...
    <main class="flex-grow p-4 sm:p-8 w-full max-w-[1600px] mx-auto space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
            <div class="p-6 border-b border-slate-100">
                ...
                <p class="text-xs text-slate-500 font-medium">Pelanggan diurutkan berdasarkan total nilai nominal belanja terbanyak. Klik baris pelanggan untuk melihat riwayat transaksi detail.</p>
            </div>
                ...
                <table class="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                        <tr class="bg-slate-50/50 text-[10px] font-bold text-slate-400 uppercase tracking-widest font-outfit border-b border-slate-100">
                            ...
                            <th class="py-4 px-6 text-right">Rata-rata Belanja (AOV)</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700 text-sm font-medium">
                        ...
                        {:else}
                            {#each customers.data as item}
                                <tr class="hover:bg-slate-50/50 transition cursor-pointer" onclick={() => openCustomerDetail(item)}>
                                    ...
                                    <td class="py-4 px-6 text-right font-bold text-slate-700">{formatRupiah(item.average_order_value)}</td>
                                    <td class="py-4 px-6 text-right font-bold text-rose-600">-{formatRupiah(item.total_discounts)}</td>
                                    <td class="py-4 px-6 text-right font-bold text-amber-600">{item.total_coins_redeemed} koin</td>
...
{#if showModal && selectedCustomer}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <!-- Modal Box -->
        <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden z-10 animate-in fade-in zoom-in duration-200 flex flex-col max-h-[85vh]">
            
            <!-- Modal Header -->
            <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50 shrink-0">
                <div class="flex items-center gap-3">
                    <span class="w-10 h-10 rounded-2xl flex items-center justify-center text-xl text-blue-600 bg-blue-50">
                        <i class="ti ti-user"></i>
                    </span>
                    <div>
                        <h3 class="font-outfit font-black text-lg text-slate-800 leading-tight">
                            Riwayat Penjualan: {selectedCustomer.name}
                        </h3>
                        <p class="text-xs text-slate-400 font-medium">
                            {selectedCustomer.email} &bull; Bergabung pada {formatDate(selectedCustomer.registered_at)}
                        </p>
                    </div>
                </div>
                <button 
            ...
            <div class="p-6 overflow-y-auto flex-grow space-y-4">
                {#if isLoadingTransactions}
                    <div class="py-12 flex flex-col items-center justify-center text-slate-400 space-y-3">
                        ...
                        <span class="text-xs font-bold font-outfit uppercase tracking-wider">Memuat riwayat transaksi...</span>
                    </div>
                ...
                {:else}
                        ...
                        <table class="w-full text-left border-collapse min-w-[700px]">
                            <thead>
                                <tr class="bg-slate-50/50 text-[10px] font-bold text-slate-400 uppercase tracking-widest font-outfit border-b border-slate-100">
                                    ...
                                    <th class="py-3 px-4 text-right">Koin Ditukar</th>
                            ...
                            <tbody class="divide-y divide-slate-100 text-slate-700 text-xs font-medium">
                                {#each transactionsData.data as tx}
                                    <tr class="hover:bg-slate-50/30 transition animate-in fade-in duration-150">
                                        <td class="py-3 px-4 font-mono font-bold text-slate-800">{tx.transaction_number}</td>
                                        <td class="py-3 px-4 text-slate-500">{formatDate(tx.created_at)}</td>
                                        <td class="py-3 px-4">
                                            <span class="px-2 py-0.5 bg-slate-100 rounded-lg text-slate-600 font-bold border border-slate-200">
                                                {tx.payment_method?.name || '-'}
                                            </span>
                                        </td>
                                        <td class="py-3 px-4 text-center font-bold text-slate-600">{tx.items_count} pcs</td>
                                        <td class="py-3 px-4 text-right text-rose-500 font-semibold">
                                            {tx.discount_amount > 0 ? `-${formatRupiah(tx.discount_amount)}` : '-'}
                                        </td>
                                        <td class="py-3 px-4 text-right text-amber-600 font-semibold">
                                            {tx.coins_redeemed > 0 ? `${tx.coins_redeemed} koin` : '-'}
                                        </td>
                                        <td class="py-3 px-4 text-right font-bold text-slate-800">{formatRupiah(tx.grand_total)}</td>
                                        <td class="py-3 px-4 text-center">
                                            <span class="inline-block px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider {getStatusBadgeClass(tx.status)}">
                    ...
                    <!-- Modal Pagination -->
                    <div class="flex items-center justify-between mt-4 px-2 py-3 bg-slate-50/50 border-t border-slate-100 shrink-0">
                        <span class="text-[11px] text-slate-400 font-semibold uppercase tracking-wider">
                            Halaman {transactionsData.current_page} dari {transactionsData.last_page} ({transactionsData.total} Transaksi)
                        </span>
                        <div class="flex gap-2">
                            <button 
                                disabled={transactionsData.current_page === 1}
                                onclick={() => fetchTransactions(selectedCustomer.id, transactionsData.current_page - 1)}
