...
<script lang="ts">
    ...
    function exportToCSV() {
        ...
        link.setAttribute(
            'download',
            `laporan_laba_rugi_${dateFrom}_sd_${dateTo}.csv`,
        );
...
<AdminLayout>
    <main
        class="flex-grow p-4 sm:p-8 w-full max-w-full mx-auto space-y-6 print:p-0 print:bg-white"
    >
        <!-- Page Header -->
        <div
        ...
        >
            <div>
                ...
                <p class="text-sm text-slate-500 font-medium mt-1">
                    Lembar akuntansi pendapatan operasional, harga pokok
                    penjualan, pengeluaran, dan profitabilitas.
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div
                class="md:col-span-2 bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 flex flex-col justify-between"
            >
                <div>
                    <h3
                        class="font-outfit font-black text-lg text-slate-800 mb-6 border-b border-slate-100 pb-3 uppercase tracking-wider text-xs font-bold text-slate-400"
                    >
                        Lembar Akuntansi
                    </h3>
                    <div class="space-y-4 font-medium text-slate-700 text-sm">
                        ...
                        <div class="space-y-2">
                            <h4
                                class="font-black text-slate-800 border-b border-slate-100 pb-1 uppercase tracking-wider text-xs"
                            >
                                I. PENDAPATAN
                            </h4>
                            <div class="flex justify-between pl-4">
                                <span class="text-slate-600"
                                    >Penjualan Produk (Omset Subtotal)</span
                                >
                            ...
                            <div class="flex justify-between pl-4">
                                <span class="text-slate-600"
                                    >Pendapatan Biaya Admin</span
                                >
                                <span class="font-semibold text-slate-800"
                                    >{formatRupiah(
                                        metrics.admin_fee_revenue,
                                    )}</span
                                >
                            </div>
                            <div
                                class="flex justify-between pl-4 border-t border-dashed border-slate-100 pt-2 font-bold"
                            >
                                <span class="text-slate-800 uppercase text-xs"
                                    >Total Pendapatan Kotor</span
                                >
                                <span class="text-slate-900"
                                    >{formatRupiah(metrics.total_revenue)}</span
                                >
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 pt-4">
                            ...
                            <div class="flex justify-between pl-4">
                                <span class="text-slate-600"
                                    >HPP Barang Terjual (COGS)</span
                                >
                                <span class="font-semibold text-rose-500"
                                    >- {formatRupiah(metrics.total_cogs)}</span
                                >
                            </div>
                            <div
                                class="flex justify-between pl-4 border-t border-dashed border-slate-100 pt-2 font-bold"
                            >
                                <span class="text-slate-800 uppercase text-xs"
                                    >Total Harga Pokok Penjualan</span
                                >
                        ...
                        <!-- LABA KOTOR -->
                        <div
                            class="bg-slate-50 rounded-2xl p-4 flex justify-between items-center font-black mt-6 border border-slate-100"
                        >
                            <span
                                class="text-slate-800 uppercase tracking-wider text-xs"
                                >III. LABA KOTOR (GROSS PROFIT)</span
                            >
                            <span class="text-slate-900 text-base"
                                >{formatRupiah(metrics.gross_profit)}</span
                            >
                        </div>
                        ...
                        <div class="space-y-2 pt-4">
                            <h4
                                class="font-black text-slate-800 border-b border-slate-100 pb-1 uppercase tracking-wider text-xs"
                            >
                                IV. BEBAN & PENGELUARAN
                            </h4>
                            <div class="flex justify-between pl-4">
                                <span class="text-slate-600"
                                    >Subsidi Diskon Voucher Toko</span
                                >
                                <span class="font-semibold text-rose-500"
                                    >- {formatRupiah(
                                        metrics.voucher_discounts,
                                    )}</span
                                >
                            </div>
                            <div class="flex justify-between pl-4">
                                <span class="text-slate-600"
                                    >Subsidi Ongkos Kirim</span
                                >
                                <span class="font-semibold text-rose-500"
                                    >- {formatRupiah(
                                        metrics.shipping_discounts,
                                    )}</span
                                >
                            </div>
                            <div
                        ...
                        <!-- LABA BERSIH -->
                        <div
                            class="bg-emerald-50 rounded-2xl p-5 flex justify-between items-center font-black mt-8 border border-emerald-100 text-emerald-800"
                        >
                            <span class="uppercase tracking-wider text-xs"
                                >V. LABA BERSIH (NET PROFIT)</span
                            >
                            <span class="text-lg"
                                >{formatRupiah(metrics.net_profit)}</span
                            >
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6 print:hidden">
                ...
                >
                    <h4
                        class="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-outfit mb-1"
                    >
                        Rasio Margin Laba Bersih
                    </h4>
                    <h3
                        class="font-outfit font-black text-3xl text-emerald-600"
                    >
                        {metrics.profit_margin.toFixed(2)}%
                    </h3>
                    <div
                        class="w-full bg-slate-100 h-2 rounded-full mt-3 overflow-hidden"
                    >
                        <div
                            class="bg-emerald-500 h-full rounded-full"
                            style="width: {Math.max(
                                0,
                                Math.min(100, metrics.profit_margin),
                            )}%"
                        ></div>
                    </div>
                    <p class="text-[11px] text-slate-400 mt-2 font-medium">
                        Persentase laba bersih yang diserap dari total omset
                        kotor.
                    </p>
                </div>
                ...
                >
                    <h4
                        class="font-outfit font-black text-sm text-slate-800 mb-4"
                    >
