<script lang="ts">
    ...
    function doConfirmReceipt() {
        saving = true;
        router.post(
            `/admin/returns/${ret.id}/confirm-receipt`,
    ...
    function doSaveCustomerTracking() {
        ...
        saving = true;
        router.post(
            `/admin/returns/${ret.id}/customer-tracking`,
            {
            ...
            {
                onSuccess: () => {
                    showCustomerTrackingModal = false;
                    showToast('Resi customer disimpan!', 'success');
                },
                onError: (e: any) => {
                    showToast(
                        (Object.values(e)[0] as string) ?? 'Gagal.',
    ...
    function doSaveReplacementTracking() {
        ...
        saving = true;
        router.post(
            `/admin/returns/${ret.id}/replacement-tracking`,
            {
            ...
            {
                onSuccess: () => {
                    showReplacementTrackingModal = false;
                    showToast('Resi penggantian disimpan!', 'success');
                },
                onError: (e: any) => {
                    showToast(
                        (Object.values(e)[0] as string) ?? 'Gagal.',
    ...
    <div class="flex-grow p-4 sm:p-8 w-full max-w-full mx-auto">
        ...
        <div class="flex items-start gap-4 mb-6">
            <a
                href="/admin/returns"
                aria-label="Kembali"
                class="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-50 transition shrink-0"
            >
            ...
            <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    {#if ret.type === 'refund'}
                        <span
                            class="text-[10px] font-black px-2.5 py-1 rounded-full bg-cyan-50 text-cyan-700 border border-cyan-200"
                        >
                            💰 Pengembalian Dana
                        </span>
                    {:else}
                        <span
                            class="text-[10px] font-black px-2.5 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-200"
                        >
                            📦 Penggantian Barang
                        </span>
                    {/if}
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                >
                    ...
                    >
                        <i class="ti ti-user text-base text-slate-400"></i> Informasi
                        Customer
                    </h4>
                    ...
                    <div class="p-3 bg-slate-50 rounded-xl">
                        ...
                        <p class="text-xs text-slate-500 mt-0.5">
                            Grand Total: <strong
                                >{fmt(ret.transaction?.grand_total)}</strong
                            >
                        </p>
                    </div>
                ...
                >
                    <h4
                        class="font-bold text-slate-800 text-sm mb-3 flex items-center gap-2"
                    >
                        <i class="ti ti-message-dots text-base text-slate-400"
                        ></i> Alasan Retur
                    </h4>
                    <p
                        class="text-sm text-slate-700 bg-slate-50 rounded-xl p-4 leading-relaxed"
                    >
                        {ret.reason}
                    </p>
                    ...
                    {#if ret.notes_admin}
                        <div
                            class="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-xl"
                        >
                            <p
                ...
                >
                    <h4
                        class="font-bold text-slate-800 text-sm mb-4 flex items-center gap-2"
                    >
                        <i class="ti ti-package-import text-base text-slate-400"
                        ></i> Produk Diretur
                    </h4>
                    <div class="space-y-3">
                        {#each ret.items ?? [] as item (item.id)}
                            ...
                            >
                                <div
                                    class="w-10 h-10 rounded-lg bg-slate-200 flex items-center justify-center shrink-0"
                                >
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    {#if item.variant_name}
                                        <span
                                            class="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-1.5 py-0.5 rounded mt-0.5 inline-block"
                                ...
                                <div class="text-right shrink-0">
                                    <p class="text-xs font-bold text-slate-500">
                                        x{item.quantity_returned}
                                    </p>
                                    <p
                                        class="text-sm font-black text-slate-800"
                                    >
                                        {fmt(item.refund_subtotal)}
                                    </p>
                                </div>
                            </div>
                        {/each}
                    ...
                    <div
                        class="flex justify-between items-center pt-3 border-t border-slate-100 mt-3"
                    >
                        <span class="text-sm font-bold text-slate-500"
                            >Total Nominal</span
                        >
                        <span class="text-lg font-black" style="color:{primary}"
                ...
                >
                    <h4
                        class="font-bold text-slate-800 text-sm mb-4 flex items-center gap-2"
                    >
                        <i class="ti ti-photo text-base text-slate-400"></i> Bukti
                        Retur (Foto/Video)
                    </h4>
                    {#if (ret.media ?? []).length === 0}
                        <p class="text-xs text-slate-400 italic">
                            Tidak ada bukti media.
                        </p>
                    {:else}
                            ...
                            {#each ret.media ?? [] as media, idx (media.id)}
                                ...
                                >
                                    {#if media.file_type === 'video'}
                                        ...
                                        >
                                            <i
                                                class="ti ti-player-play text-white text-2xl"
                                            ></i>
                                        </div>
                                        <span
                                            class="absolute bottom-1 right-1 text-[9px] bg-black/60 text-white px-1.5 py-0.5 rounded font-bold"
                                            >VIDEO</span
                                        >
                                    {:else}
                                        <img
            ...
            <div class="space-y-5">
                ...
                {#if ret.type === 'refund'}
                    <div
                        class="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-2xl border border-cyan-200 p-5"
                    >
                        <h4
                            class="font-bold text-cyan-800 text-sm mb-3 flex items-center gap-2"
                        >
                            <i class="ti ti-building-bank text-base"></i> Rekening
                            Refund Customer
                        </h4>
                        {#if primaryBankAccount}
                            <div
                                class="bg-white rounded-xl p-4 border border-cyan-200"
                            >
                                <p
                                ...
                                <p
                                    class="font-black text-slate-800 text-base mt-0.5"
                                >
                                    {primaryBankAccount.bank_name}
                                </p>
                                <p
                                    class="font-mono font-bold text-lg text-slate-900 mt-1 tracking-wider"
                                >
                                    {primaryBankAccount.account_number}
                                </p>
                                <p class="text-xs text-slate-500 mt-0.5">
                                    a.n. {primaryBankAccount.account_name}
                                </p>
                            </div>
                            <div
                                class="mt-3 p-3 bg-white rounded-xl border border-cyan-200"
                            >
                        ...
                        {:else}
                            <div
                                class="bg-white/70 rounded-xl p-4 border border-dashed border-cyan-300 text-center"
                            >
                                <i
                                    class="ti ti-alert-circle text-amber-500 text-xl mb-1 block"
                                ></i>
                                <p class="text-xs text-slate-500 font-bold">
                                    Customer belum mengisi rekening bank.
                                </p>
                                <p class="text-[10px] text-slate-400 mt-0.5">
                                    Hubungi customer untuk info rekening.
                                </p>
                            </div>
                            <div
                                class="mt-3 p-3 bg-white rounded-xl border border-cyan-200"
                ...
                >
                    <h4
                        class="font-bold text-slate-800 text-sm mb-4 flex items-center gap-2"
                    >
                        <i class="ti ti-truck-delivery text-base text-slate-400"
                        ></i> Nomor Resi
                    </h4>
                    ...
                    <div class="mb-4">
                        <p
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5"
                        >
                            Resi Barang Retur (dari Customer)
                        </p>
                        {#if ret.return_tracking_number}
                            <div class="p-3 bg-slate-50 rounded-xl">
                                <p
                                    class="font-mono font-bold text-sm text-slate-800"
                                >
                        ...
                        {#if ['disetujui', 'barang_dikirim_customer'].includes(ret.status)}
                            <button
                                onclick={() => {
                                    ...
                                    showCustomerTrackingModal = true;
                                }}
                    ...
                    {#if ret.type === 'penggantian_barang'}
                        <div>
                            <p
                                class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5"
                            >
                                Resi Barang Pengganti (ke Customer)
                            </p>
                            {#if ret.replacement_tracking_number}
                                <div
                                    class="p-3 bg-purple-50 rounded-xl border border-purple-200"
                                >
                                    <p
                                        class="font-mono font-bold text-sm text-purple-800"
                                    >
                                        {ret.replacement_tracking_number}
                                    </p>
                                    {#if ret.replacement_courier_name}
                                        <p
                                            class="text-[10px] text-purple-400 mt-0.5"
                                        >
                                            {ret.replacement_courier_name}
                                        </p>
                                    {/if}
                                </div>
                            {:else}
                            ...
                            {#if ['selesai'].includes(ret.status) || ret.replacement_transaction_id}
                                <button
                                    onclick={() => {
                                        ...
                                        replacementCourier =
                                            ret.replacement_courier_name ?? '';
                                        showReplacementTrackingModal = true;
                                    }}
                                    class="mt-2 text-xs font-bold underline"
                                    style="color:{primary}"
                                >
                ...
                >
                    <h4
                        class="font-bold text-slate-800 text-sm mb-2 flex items-center gap-2"
                    >
                        <i class="ti ti-settings text-base text-slate-400"></i> Aksi
                        Admin
                    </h4>
                    ...
                    {#if ret.status === 'barang_diterima_toko'}
                        {#if ret.type === 'refund'}
                            <div class="space-y-2">
                                ...
                                >
                                    <i class="ti ti-cash-banknote text-base"
                                    ></i>
                                    {saving
                                        ? 'Memproses...'
                    ...
                    {#if ret.status === 'selesai' || ret.status === 'ditolak'}
                        <div class="text-center py-4">
                            <i
                                class="ti ti-circle-check text-3xl {ret.status ===
                                'selesai'
                                    ? 'text-emerald-400'
                                    : 'text-red-400'}"
                            ></i>
                ...
                >
                    ...
                    <div class="space-y-3 text-xs">
                        <div class="flex gap-3">
                            <div
                                class="w-2 h-2 rounded-full bg-slate-400 shrink-0 mt-1"
                            ></div>
                            <div>
                        ...
                        {#if ret.approved_at}
                            <div class="flex gap-3">
                                <div
                                    class="w-2 h-2 rounded-full bg-blue-400 shrink-0 mt-1"
                                ></div>
                                <div>
                                    <p class="font-bold text-blue-700">
                                        Disetujui oleh {ret.approved_by_user
                                            ?.name ?? 'Admin'}
                                    </p>
                                    <p class="text-slate-400">
                                        {fmtDate(ret.approved_at)}
                                    </p>
                                </div>
                            </div>
                        {/if}
                        {#if ret.rejected_at}
                            <div class="flex gap-3">
                                <div
                                    class="w-2 h-2 rounded-full bg-red-400 shrink-0 mt-1"
                                ></div>
                                <div>
                        ...
                        {#if ret.received_at}
                            <div class="flex gap-3">
                                <div
                                    class="w-2 h-2 rounded-full bg-purple-400 shrink-0 mt-1"
                                ></div>
                                <div>
                                    <p class="font-bold text-purple-700">
                                        Barang Diterima Toko
                                    </p>
                                    <p class="text-slate-400">
                                        {fmtDate(ret.received_at)}
                                    </p>
                                </div>
                            </div>
                        {/if}
                        {#if ret.refunded_at}
                            <div class="flex gap-3">
                                <div
                                    class="w-2 h-2 rounded-full bg-cyan-400 shrink-0 mt-1"
                                ></div>
                                <div>
    ...
    >
        ...
        >
            ...
            <div class="flex gap-3">
                <button
                    onclick={() => (showApproveModal = false)}
                    class="flex-1 py-3 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50"
                    >Batal</button
                >
                <button
                    onclick={doApprove}
                    disabled={saving}
                    class="flex-1 py-3 rounded-xl text-white font-bold text-sm bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50"
                >
...
{#if showReceiptModal}
    ...
    >
        ...
        >
            ...
            <p
                class="text-xs text-slate-400 font-bold mb-5 uppercase tracking-wider"
            >
                Pilih Opsi Tindakan Stok Produk
            </p>
            ...
            <div class="space-y-3 mb-6">
                ...
                <button
                    ...
                    class="w-full text-left p-4 rounded-xl border-2 transition flex items-start gap-3 cursor-pointer {stockAction ===
                    'active'
                        ? 'bg-emerald-50/20'
                        : 'bg-white hover:bg-slate-50 border-slate-100'}"
                    style="border-color: {stockAction === 'active'
                        ? '#10b981'
                        : '#e2e8f0'}"
                >
                    <div
                        ...
                        style="border-color: {stockAction === 'active'
                            ? '#10b981'
                            : '#cbd5e1'}; color: {stockAction === 'active'
                            ? '#10b981'
                            : 'transparent'}"
                    >
                    ...
                    <div>
                        <span class="text-xs font-black text-slate-800 block"
                            >Kembalikan ke Stok Aktif</span
                        >
                        <span
                            class="text-[10px] text-slate-400 font-bold leading-normal block mt-1"
                            >Barang layak jual kembali. Stok produk aktif di
                            sistem akan bertambah otomatis.</span
                        >
                    </div>
                </button>
                ...
                <button
                    ...
                    'damaged'
                        ? 'bg-rose-50/20'
                        : 'bg-white hover:bg-slate-50 border-slate-100'}"
                    style="border-color: {stockAction === 'damaged'
                        ? '#ef4444'
                        : '#e2e8f0'}"
                >
                    <div
                        ...
                        style="border-color: {stockAction === 'damaged'
                            ? '#ef4444'
                            : '#cbd5e1'}; color: {stockAction === 'damaged'
                            ? '#ef4444'
                            : 'transparent'}"
                    >
                        <div class="w-2.5 h-2.5 rounded-full bg-current"></div>
                    ...
                    <div>
                        <span class="text-xs font-black text-slate-800 block"
                            >Catat sebagai Stok Rusak</span
                        >
                        <span
                            class="text-[10px] text-slate-400 font-bold leading-normal block mt-1"
                            >Barang cacat/rusak dan tidak layak jual. Stok
                            produk aktif di sistem TIDAK akan bertambah.</span
                        >
                    </div>
                </button>
            </div>
        ...
        >
            ...
            <div class="flex gap-3">
                ...
                <button
                    onclick={doReject}
                    disabled={saving}
                    class="flex-1 py-3 rounded-xl text-white font-bold text-sm bg-red-500 hover:bg-red-600 disabled:opacity-50"
                >
...
{#if showCustomerTrackingModal}
    ...
    <div
        class="fixed inset-0 z-50 flex items-center justify-center p-4"
        onclick={() => (showCustomerTrackingModal = false)}
    >
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <!-- svelte-ignore a11y_click_events_have_key_events -->
        ...
        >
            <h3 class="font-bold text-slate-800 text-lg mb-4">
                Resi Pengiriman Barang Retur (Customer)
            </h3>
            <div class="space-y-4">
                <div>
                    <p class="block text-xs font-bold text-slate-600 mb-2">
                    ...
                    <input
                        ...
                        class="w-full px-4 py-3 text-sm border border-slate-200 rounded-xl focus:outline-none font-mono"
                    />
                </div>
                <div>
                    <p class="block text-xs font-bold text-slate-600 mb-2">
                    ...
                    <input
                        ...
                        class="w-full px-4 py-3 text-sm border border-slate-200 rounded-xl focus:outline-none"
                    />
                </div>
            </div>
            <div class="flex gap-3 mt-5">
                <button
                    onclick={() => (showCustomerTrackingModal = false)}
                    class="flex-1 py-3 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm"
                    >Batal</button
                >
                <button
                    onclick={doSaveCustomerTracking}
                    disabled={saving}
...
{#if showReplacementTrackingModal}
    ...
    <div
        class="fixed inset-0 z-50 flex items-center justify-center p-4"
        onclick={() => (showReplacementTrackingModal = false)}
    >
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <!-- svelte-ignore a11y_click_events_have_key_events -->
        ...
        >
            ...
            <div class="space-y-4">
                <div>
                    <p class="block text-xs font-bold text-slate-600 mb-2">
                        Nomor Resi Pengganti <span class="text-red-500">*</span>
                    </p>
                    <input
                        type="text"
                        bind:value={replacementTracking}
                        placeholder="Contoh: JNE1234567890"
                        class="w-full px-4 py-3 text-sm border border-slate-200 rounded-xl focus:outline-none font-mono"
                    />
                </div>
            ...
            <div class="flex gap-3 mt-5">
                ...
                >
                <button
                    onclick={doSaveReplacementTracking}
                    disabled={saving}
                    class="flex-1 py-3 rounded-xl text-white font-bold text-sm disabled:opacity-50"
                    style="background:{primary}"
                >
            ...
            {#key previewIndex}
                ...
                >
                    {#if previewItems[previewIndex].file_type === 'video'}
                        <video
                            src={formatMediaUrl(
                                previewItems[previewIndex].file_path,
                            )}
                            controls
                            autoplay
            ...
            >
                {#each previewItems as item, idx}
                    ...
                    >
                        ...
                        {:else}
                            <img
                                src={formatMediaUrl(item.file_path)}
                                alt="Thumb"
