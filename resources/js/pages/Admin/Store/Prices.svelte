...
<svelte:head>
    <title>Atur Harga & HPP — Admin</title>
</svelte:head>
...
<AdminLayout>
    <main class="w-full max-w-[1600px] mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-semibold tracking-tight text-slate-900">Atur Harga & HPP</h1>
                <p class="mt-0.5 text-sm text-slate-550">Kelola harga jual, harga modal, dan detail kalkulasi pajak produk & varian secara massal</p>
            </div>
        </div>

        <!-- Navigation Tabs -->
        <div class="flex border-b border-slate-200 w-full">
            <a
                href="/admin/store/prices"
                use:inertia
                class="px-4 py-2 border-b-2 font-medium text-sm transition-colors cursor-pointer {page.url.startsWith('/admin/store/prices')
            ...
            <a
                ...
                use:inertia
                class="px-4 py-2 border-b-2 font-medium text-sm transition-colors cursor-pointer {page.url.startsWith('/admin/store/stocks')
                    ? 'border-brand-blueRoyal text-brand-blueRoyal'
                    : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}"
            >
                Atur Stok
            ...
            <a
                ...
                use:inertia
                class="px-4 py-2 border-b-2 font-medium text-sm transition-colors cursor-pointer {page.url.startsWith('/admin/store/shipping')
                    ? 'border-brand-blueRoyal text-brand-blueRoyal'
                    : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}"
            >
                Atur Pengiriman
            ...
            <!-- Search Bar Card -->
            <div
                class="bg-white rounded-xl border border-slate-200 p-4 shadow-xs flex flex-col sm:flex-row items-center gap-4 justify-between"
            >
                <div class="relative w-full sm:max-w-md">
                    ...
                    <input
                        ...
                        class="w-full pl-10 pr-4 py-2 text-sm bg-slate-50 border border-slate-200 focus:bg-white focus:ring-2 focus:ring-brand-blueRoyal/20 focus:border-brand-blueRoyal rounded-lg outline-none transition"
                    />
                </div>
                <div
            ...
            {#if localProducts.length === 0}
                <div
                    class="bg-white rounded-xl border border-slate-200 p-12 text-center shadow-xs"
                >
                    <div
            ...
            {:else}
                ...
                >
                    {#each localProducts as product (product.id)}
                        <div
                            class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-xs transition-all"
                        >
                            <!-- Product Row Header -->
                            <div
                            ...
                            >
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4
                                            class="font-semibold text-slate-900 text-base"
                                        >
                                            {product.name}
                                        </h4>
                                        <span
                                            class="px-2 py-0.5 bg-slate-100 text-slate-500 font-semibold text-[10px] uppercase tracking-wider rounded-md"
                                ...
                                <div class="flex items-center gap-4">
                                    {#if product.has_variants}
                                        ...
                                        <div
                                            class="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-2.5 py-1 shadow-xs"
                                        >
                                            <span
                                        ...
                                        {#if product.use_custom}
                                            <button
                                                ...
                                                onclick={() =>
                                                    (product.expanded =
                                                        !product.expanded)}
                                                class="h-8 px-3 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-650 hover:bg-slate-50 flex items-center gap-1.5 transition cursor-pointer"
                                            >
                                                <span
                            ...
                            {#if !product.has_variants || !product.use_custom}
                                ...
                                >
                                    ...
                                    <div
                                        class="bg-slate-50 rounded-xl p-5 border border-slate-100 flex flex-col justify-center"
                                    >
                                        <span
                                    ...
                                    {:else}
                                        ...
                                        >
                                            {#each product.tier_prices as tier, idx (idx)}
                                                <div
                                                    class="flex items-center gap-3 bg-slate-50/50 p-3 rounded-xl border border-slate-100 relative group"
                                                >
                                ...
                                >
                                    ...
                                    <div class="grid grid-cols-1 gap-4">
                                        {#each product.variants as variant (variant.id)}
                                            ...
                                            <div
                                                class="bg-white border border-slate-200 rounded-xl p-4 flex flex-col gap-4 shadow-sm"
                                            >
                                                <div
