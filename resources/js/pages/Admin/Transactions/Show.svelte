                    ...
                    {#if transaction.status !== 'selesai' && transaction.status !== 'batal'}
                        {#if transaction.shipping_courier === 'self_pickup'}
                            <button
                                ...
                                disabled={isUpdating}
                                class="px-5 py-2.5 rounded-xl text-xs font-bold text-white transition font-outfit uppercase tracking-wider shadow-lg flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50"
                                style="box-shadow: 0 4px 12px -2px rgba(16, 185, 129, 0.4);"
                            >
                                <i class="ti ti-check text-sm"></i>
                                Konfirmasi Pengambilan
                            </button>
                        {:else if transaction.shipping_courier !== 'digital' && transaction.shipping_courier !== 'store_courier'}
                            <button
                        ...
                        <div class="divide-y divide-slate-100">
                            {#each transaction.items ?? [] as item}
                                ...
                                >
                                    ...
                                    <div class="flex-1">
                                        <p
                                            class="text-sm font-bold text-slate-800 leading-snug whitespace-pre-wrap break-words flex items-center gap-1.5"
                                        >
                                            {item.product_name}
                                            {#if item.product?.is_digital}
                                                <span
                                        ...
                                        {#if item.product?.is_digital && transaction.status !== 'selesai' && transaction.status !== 'batal'}
                                            <div
                                                class="mt-2.5 p-3 bg-blue-50/50 border border-blue-100 rounded-2xl"
                                            >
                                                <label
                                                    for="digital_note_{item.id}"
                                                    class="text-[10px] font-black text-blue-700 uppercase tracking-wider block mb-1.5 flex items-center gap-1.5"
                                                >
                                                    <i class="ti ti-key text-xs"
                                                    ></i> Kirim Info Produk Digital
                                                    (Link / Lisensi)
                                                </label>
                            ...
                            <div class="space-y-4">
                                    ...
                                    >
                                        ...
                                        {:else}
                                            <button
                                                ...
                                                class="px-2 py-1 bg-emerald-500 text-white rounded-lg text-[10px] font-bold hover:bg-emerald-600 transition disabled:opacity-50"
                                            >
                                                Buat Kode Booking
                                            </button>
                                        {/if}
                                        ...
                                        {:else}
                                            ...
                                            >
                                                ...
                                                <button
                                                    ...
                                                    class="px-2 py-1 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-[10px] font-bold transition shrink-0"
                                    ...
                                    <button
                                        onclick={() =>
                                            updateTransactionStatus(
                                                'out_for_pickup',
                                                'Status diperbarui menjadi Out for Pickup.',
                                            )}
                                        disabled={isUpdating}
                                ...
                                <!-- <div class="mt-4 pt-4 border-t border-slate-100 space-y-3.5">
                                    ...
                                    {#if transaction.status_histories && transaction.status_histories.length > 0}
                                        <div class="relative pl-3 border-l border-slate-100 space-y-3.5 py-1 max-h-48 overflow-y-auto">
                                            {#each transaction.status_histories as step}
                                                <div class="relative">
                                                    ...
                                                    <div>
                                                        <p class="text-[9px] text-slate-400 font-bold">
                                                            {new Date(step.created_at).toLocaleString('id-ID', {
                                                                day: 'numeric',
                                                                month: 'short',
                                                                year: 'numeric',
                                                                hour: '2-digit',
                                    ...
                                    {:else}
                                        <p class="text-[10px] text-slate-400 italic">Belum ada riwayat pengiriman.</p>
                                    {/if}
                                    ...
                                    {#if transaction.status !== 'selesai' && transaction.status !== 'batal'}
                                        <div class="flex items-center gap-1.5 pt-2">
                                            <input
                                                ...
                                                placeholder="Tambah catatan riwayat..."
                                                class="flex-1 px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-slate-300 bg-white transition"
                                            />
                                            <button
                                                onclick={addCustomLog}
                                                disabled={storeActionLoading || !customDeliveryLog.trim()}
                                                class="px-3 py-1.5 bg-slate-800 text-white rounded-lg text-[10px] font-bold hover:bg-slate-900 transition disabled:opacity-50 shrink-0 font-outfit uppercase tracking-wider"
                                            >
                                                Tambah
                                            </button>
                                        </div>
                            ...
                            >
                                ...
                                <div class="space-y-4">
                                    {#if !transaction.booking_code}
                                        ...
                                        >
                                            <i
                                                class="ti ti-package-export text-sm"
                                            ></i>
                                            {bookingLoading
                                                ? 'Memproses Booking...'
                        ...
                        {#if transaction.shipping_courier !== 'digital'}
                            {#if transaction.shipping_courier === 'self_pickup'}
                                ...
                                >
                                    ...
                                    >
                                        <i
                                            class="ti ti-building-store text-base"
                                        ></i>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            memverifikasi pengambilan barang
                                            oleh customer.
                                        </p>
                                    </div>
                                    ...
                                    {#if transaction.status === 'dikemas'}
                                        <button
                                            onclick={() =>
                                                updateTransactionStatus(
                                                    'out_for_pickup',
                                                    'Status diperbarui menjadi Out for Pickup (Siap Diambil). Email pemberitahuan telah dikirim.',
                                                )}
                                            disabled={isUpdating}
                                    ...
                                    {#if transaction.status === 'out_for_pickup'}
                                        ...
                                        >
                                            <i
                                                class="ti ti-circle-check text-sm"
                                            ></i>
            ...
            >
                ...
                >
                    {#each Object.entries(statusLabels) as [key, label]}
                        {@const targetIdx = statusSteps.findIndex(
                            (s) => s.key === key,
                        )}
                        <option
                            value={key}
