    ...
    async function executeStep() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const data = await response.json();
                runResult = data.result || { error: 'Terjadi kesalahan sistem.' };
...
<AdminLayout>
    <div class="flex-grow p-6 flex flex-col h-[calc(100vh-5rem)] min-h-0 overflow-hidden w-full max-w-[1600px] mx-auto bg-white">
        <!-- Header -->
        <div class="flex items-center gap-3 pb-5 shrink-0">
            ...
            <h1 class="font-outfit font-black text-lg text-slate-800">User Acceptance Testing</h1>
        </div>
                ...
                {#each steps as step, idx}
                    ...
                    >
                        <div
                            ...
                            style="background-color: {idx <= activeStepIndex ? '#fa4a2a' : '#8d9196'};"
                        >
                            {idx + 1}
                        </div>
                        <span
                            class="text-xs font-bold transition-colors duration-300"
                            style="color: {idx <= activeStepIndex ? '#fa4a2a' : '#8d9196'};"
                        >
                            {step.label}
                        </span>
                    </button>
        ...
        <div class="flex-grow flex flex-col min-h-0">
            ...
            <div class="mb-5 shrink-0">
                <h2 class="text-sm font-bold text-slate-800 mb-2">{currentStep.label}</h2>
            ...
            <div class="flex-grow overflow-y-auto min-h-0 custom-scrollbar pr-2">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-[11px] font-bold text-slate-900 border-b border-slate-200">
                    ...
                    <tbody class="text-xs align-top">
                        <tr>
                            <td class="py-4 pr-3 text-slate-500 italic font-normal leading-relaxed text-[11px]">
                                {currentStep.name}
                            </td>
                            <td class="py-4 pr-3 text-slate-700 font-normal text-[11px] leading-relaxed">
                                {currentStep.input}
                            </td>
                            <td class="py-4 pr-3 text-slate-600 font-mono text-[10px] whitespace-pre-wrap leading-relaxed">
                                {currentStep.expect}
                            </td>
                            <td class="py-4 pr-3">
                                <div class="relative w-full h-[180px]">
                                    ...
                                    {#if runCurl && runCurl !== '-'}
                                        <button
                                            onclick={() => copyToClipboard(runCurl)}
                            ...
                            <td class="py-4 pr-3">
                                <div class="relative w-full h-[180px]">
                                    ...
                                    {#if runResult}
                                        <button
                                            onclick={() => copyToClipboard(JSON.stringify(runResult, null, 2))}
                                            class="absolute right-2 top-2 px-2 py-1 bg-slate-50 border border-slate-200 hover:bg-slate-100 text-[9px] font-bold text-slate-600 rounded-md cursor-pointer shadow-sm active:scale-95 transition"
                                        >
                                            Copy
                                        </button>
                            ...
                            <td class="py-4 text-slate-500 font-normal text-[11px]">
                                -
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="py-4 border-t border-slate-200 bg-white flex justify-between items-center shrink-0 mt-4">
                <div>
                    <button
                        onclick={executeStep}
                        disabled={isLoading}
                ...
                <div class="flex gap-2">
                    <button
                        onclick={() => activeStepIndex = Math.max(0, activeStepIndex - 1)}
                    ...
                    <button
                        onclick={() => activeStepIndex = Math.min(steps.length - 1, activeStepIndex + 1)}
