...
>
    <!-- Left Panel: Branding -->
    <div
        class="hidden lg:flex relative overflow-hidden items-center justify-center p-12"
        style="background: linear-gradient(135deg, {primaryColor}, #1e40af, #312e81);"
    >
        <!-- Animated Background -->
        <div class="absolute inset-0 z-0 overflow-hidden">
            <!-- Decorative circles -->
            <div
                class="absolute -top-20 -left-20 w-96 h-96 rounded-full opacity-10 bg-white blur-3xl"
            ></div>
            <div
                class="absolute -bottom-20 -right-20 w-80 h-80 rounded-full opacity-10 bg-white blur-3xl"
            ></div>
            <div
                class="absolute top-1/3 right-1/4 w-40 h-40 rounded-full opacity-5 bg-white blur-2xl"
            ></div>
        </div>

        <!-- Floating delivery illustration cards -->
        <div
            class="absolute top-16 right-12 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 animate-bounce"
            style="animation-duration: 3s;"
        >
            <i class="ti ti-package text-white text-3xl"></i>
        </div>
        <div
            class="absolute bottom-24 left-10 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 animate-bounce"
            style="animation-duration: 4s; animation-delay: 0.5s;"
        >
            <i class="ti ti-map-pin text-white text-3xl"></i>
        </div>
        <div
            class="absolute top-1/2 right-8 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-3 animate-bounce"
        ...
        >
            {#if storeLogo}
                <img
                    src={storeLogo}
                    alt="Store Logo"
                    class="w-16 h-16 object-contain mb-8 rounded-3xl shadow-2xl bg-white/10 backdrop-blur-md border border-white/20 p-2 transition-transform hover:scale-105"
                />
            {:else}
                <div
            ...
            <h1
                class="text-4xl font-bold mb-6 leading-tight tracking-tight"
                style="font-family: 'Outfit', sans-serif;"
            >
                Portal Kurir Toko
            </h1>
            <p
                class="text-base text-blue-100 font-medium leading-relaxed mb-10"
            >
                Kelola pengiriman pesanan kurir toko. Scan QR, perbarui status
                pengiriman, dan kirimkan notifikasi ke pelanggan secara
                real-time.
            </p>

            <!-- Feature list -->
            <div class="space-y-3">
                {#each [{ icon: 'ti-qrcode', text: 'Scan QR & Barcode pesanan' }, { icon: 'ti-truck', text: 'Update status pengiriman langkah demi langkah' }, { icon: 'ti-bell', text: 'Notifikasi otomatis ke pelanggan' }] as feature}
                    <div
        ...
        <div class="max-w-md w-full mx-auto relative z-10 my-auto">
            ...
            <div class="flex lg:hidden items-center gap-3 mb-10">
                ...
                <div class="flex flex-col">
                    <span
                        class="font-bold text-2xl text-slate-800 tracking-tight leading-none"
                        style="font-family: 'Outfit', sans-serif;"
                        >{storeName}</span
                    >
                    <span
                        class="font-bold text-[10px] text-slate-400 tracking-widest mt-1 uppercase"
                        >Portal Kurir</span
                    >
                </div>
            </div>
            ...
            <!-- Badge -->
            <div
                class="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-full text-xs font-black mb-6 tracking-wide"
            >
                ...
                <div class="pt-4">
                    ...
                    >
                        {#if form.processing}
                            ...
                            ></i> Memproses...
                        {:else}
                            <i class="ti ti-truck-delivery mr-2 text-xl"></i> Masuk
                            ke Portal Kurir
                        {/if}
                    </button>
                </div>
            ...
            <p class="mt-8 text-center text-xs text-slate-400">
                Khusus akun berperan <span class="font-bold text-slate-600"
                    >Kurir Toko</span
                >. Bukan kurir? <Link
                    href="/login"
                    class="font-bold hover:underline"
                    style="color: {primaryColor};">Login sebagai Admin</Link
                >
            </p>
        </div>
    </div>
