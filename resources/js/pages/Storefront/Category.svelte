<script lang="ts">
    ...
    $effect(() => {
        ...
        if (categoryScrollRef) {
            requestAnimationFrame(() => {
                const activeEl = categoryScrollRef?.querySelector('[data-active="true"]');
                if (activeEl) {
    ...
    function navigateToCategory(cat: any) {
        router.get(`/category/${cat.slug || cat.id}`, {}, {
            replace: true,
    ...
    function applyFilters(closeDrawer = true) {
        ...
        router.get(`/category/${category.slug || category.id}`, {
            q: searchQ,
            min_price: minPrice,
            max_price: maxPrice,
            sort: selectedSort,
...
<svelte:head>
    <title>Kategori: {category.name || 'Kategori'} - {storeName || 'Toko Kami'}</title>
</svelte:head>
        ...
        >
            {#each categories as cat}
                <button
                    ...
                    style={cat.id === category.id ? `background-color: ${primary};` : ''}
                >
                    ...
                    <div class="space-y-1 max-h-60 overflow-y-auto pr-1">
                        {#each categories as cat}
                            <button
                                ...
                                style={cat.id === category.id ? `color: ${primary};` : ''}
                            >
                ...
                {#if allProducts.length === 0 && !isLoadingMore}
                    <div class="bg-white border border-slate-150 rounded-[28px] py-16 px-6 sm:px-12 text-center max-w-4xl mx-auto w-full transition-all duration-300">
                        ...
                        <p class="text-slate-400 text-xs sm:text-sm max-w-md mx-auto leading-relaxed mt-2 mb-8">
                            Kami tidak dapat menemukan produk yang cocok di kategori ini dengan filter Anda. Coba reset filter atau gunakan kata kunci lain.
                        </p>
