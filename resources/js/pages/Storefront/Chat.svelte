...
<StorefrontLayout hideMobileHeader={true} hideMobileFooter={true}>
    ...
    >
        ...
        <h1
            class="hidden md:flex font-outfit font-black text-2xl sm:text-3xl text-slate-800 items-center gap-2.5 mb-6"
        >
        ...
        <!-- Chat Workspace Area -->
        <div
            class="flex-grow flex bg-white border-0 md:border border-slate-200/80 rounded-none md:rounded-3xl shadow-none md:shadow-xl overflow-hidden min-h-[calc(100vh-12rem)] md:min-h-[600px] h-[calc(100vh-80px)] md:h-[650px] w-full"
        >
            ...
            >
                ...
                <div
                    class="md:hidden flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-white"
                >
                    <div class="flex items-center gap-3">
                        ...
                        <span
                            class="font-outfit font-black text-sm text-slate-800"
                            >Pusat Pesan</span
                        >
                    </div>
                    <button
                ...
                >
                    {#if filteredChats.length === 0}
                        <div class="py-16 text-center text-slate-400 px-4">
                            <i
                                class="ti ti-message-2 text-3xl mb-2 text-slate-300 block"
                            ></i>
                        ...
                        {#each filteredChats as chat (chat.id)}
                            ...
                            >
                                ...
                                <div
                                    ...
                                    style={!(page.props as any).settings
                                        ?.store_icon
            ...
            >
                {#if activeChat}
                    ...
                    >
                        ...
                        {#if activeChat.product_id}
                            <button
                                onclick={() =>
                                    router.visit(
                                        `/products/${activeChat.product_id}`,
                                    )}
                    ...
                    <!-- Messages list -->
                    <div
                        class="chat-messages-container flex-grow overflow-y-auto overflow-x-hidden px-4 py-6 pb-20 md:pb-6 space-y-4 relative"
                    >
                        <!-- Fraud Warning -->
                        <div
                            class="bg-amber-50 border border-amber-100 rounded-2xl px-4 py-3 text-xs text-amber-800 leading-relaxed text-center max-w-xl mx-auto"
                        >
                            ⚠️ Hati-hati penipuan! Selalu lakukan pembayaran
                            resmi lewat platform. Jangan bertransaksi secara
                            personal/direct transfer.
                        </div>
                        ...
                        {#each messages as msg (msg.id)}
                            <div
                                ...
                                'user'
                                    ? 'items-end'
                                    : 'items-start'} gap-1 w-full animate-fade-in"
                            >
                                <div
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {#if msg.attachment_type === 'product' && msg.attachment_data}
                                            <button
                                                type="button"
                                                class="text-left block w-full max-w-[100%] rounded-2xl overflow-hidden border shadow-sm cursor-pointer {msg.sender_type ===
                                                'user'
                                                    ? 'rounded-tr-sm'
                                                    : 'rounded-tl-sm'} border-0"
                                                style="background-color: {msg.sender_type ===
                                                'user'
                                                    ? primary
                                                    : 'white'};"
                                                onclick={() =>
                                                    msg.attachment_data.id &&
                                                    router.visit(
                                                        `/products/${msg.attachment_data.id}`,
                                                    )}
                                            >
                                                <div
                                        ...
                                        {#if msg.attachment_type === 'image' && msg.attachment_data?.url}
                                            <div
                                                class="max-w-[100%] rounded-2xl overflow-hidden border shadow-sm"
                                            >
                                                <button
                                                    type="button"
                    ...
                    <div
                        class="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-100 pb-safe md:relative md:bottom-auto md:left-auto md:right-auto md:z-0 md:border-t-0 md:pb-0 shrink-0"
                    >
                        ...
                        <div class="bg-white px-4 py-3">
                            <div class="flex items-center gap-2.5">
                                <button
                                    ...
                                    class="text-slate-400 hover:text-slate-600 w-10 h-10 flex items-center justify-center rounded-full transition hover:bg-slate-100 cursor-pointer {attachMenuOpen
                                        ? 'bg-slate-100'
                                        : ''}"
                        ...
                        {#if attachMenuOpen}
                            <div
                                class="bg-white border-t border-slate-100 px-6 py-4 shrink-0 animate-slide-up"
                            >
                                <div class="flex items-start gap-8">
                                    ...
                                    >
                                        ...
                                        >
                                            <i
                                                class="ti ti-photo text-white text-xl"
                                            ></i>
                                        </div>
                                        <span
                ...
                {:else}
                    ...
                    >
                        <div
                            class="w-20 h-20 rounded-full bg-white flex items-center justify-center shadow-md mb-4 text-slate-300"
                        >
                            <i class="ti ti-message-circle text-4xl"></i>
                        </div>
                        <h2
                            class="text-slate-800 font-outfit font-black text-lg mb-1"
                        >
                            Pusat Bantuan Chat
                        </h2>
                        <p
                            class="text-slate-400 text-xs max-w-xs leading-relaxed"
                        >
                            Silakan pilih obrolan di samping kiri atau buka
                            halaman detail produk untuk mulai chat dengan Admin
                            toko.
                        </p>
                    </div>
                {/if}
