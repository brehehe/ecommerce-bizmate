            ...
            {#if refunds.total === 0}
                <div class="w-full max-w-6xl mx-auto bg-white rounded-3xl border border-slate-100 shadow-sm p-8 py-20 flex flex-col items-center justify-center text-center">
                    <div class="w-20 h-20 rounded-full flex items-center justify-center mb-4" style="background-color: {secondary}10">
                        <i class="ti ti-receipt-refund text-3xl text-red-500"></i>
                    </div>
                    <p class="font-bold text-slate-700 text-lg mb-1">Belum ada pengajuan refund</p>
                    <p class="text-sm text-slate-400 mb-6 font-medium leading-relaxed">
                        Anda tidak memiliki pengajuan pembatalan atau refund dana aktif saat ini.
                    </p>
                    <Link
                        href="/transactions"
            ...
            {:else}
                <div class="max-w-6xl mx-auto space-y-4">
                    {#each refunds.data as refund (refund.id)}
                        {@const statusColor = refundStatusColors[refund.status] ?? { bg: '#f1f5f9', text: '#475569' }}
                        <Link
                        ...
                        >
                            ...
                            <div class="px-4 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/30">
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2.5 flex-wrap">
                                        <p class="text-sm font-black text-slate-800 tracking-tight whitespace-nowrap">#{refund.refund_number}</p>
                                    ...
                                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Diajukan: {fmtDate(refund.created_at)}</p>
                                </div>
                            ...
                            <div class="px-4 py-4 space-y-2">
                                <div class="flex justify-between items-center text-xs">
                                    <span class="text-slate-400 font-semibold">No. Transaksi:</span>
                                    <span class="text-slate-700 font-bold">#{refund.transaction?.transaction_number}</span>
                                </div>
                                <div class="flex justify-between items-start text-xs gap-4">
                                    <span class="text-slate-400 font-semibold shrink-0">Alasan Pembatalan:</span>
                                    <span class="text-slate-650 font-semibold truncate max-w-xs">{refund.reason}</span>
                                </div>
                                {#if refund.refund_method === 'transfer' && refund.bank_name}
                                    <div class="flex justify-between items-center text-xs">
                                        <span class="text-slate-400 font-semibold">Tujuan Transfer:</span>
                                        <span class="text-slate-700 font-bold">{refund.bank_name} - {refund.account_number}</span>
                                    </div>
                                {/if}
                            </div>
                            ...
                            <div class="px-4 py-3 border-t border-slate-50 bg-slate-50/20 flex items-center justify-between">
                                <div>
                                    <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Nominal Refund</span>
                                    <p class="text-sm font-black mt-0.5" style="color:{primary}">{fmt(refund.refund_amount)}</p>
                                </div>
                            ...
                            {#each refunds.links as link}
                                {#if link.url}
                                    <Link
                                        ...
                                        style={link.active ? `background-color: ${primary}` : ''}
                                    >
                                        {@html link.label}
                                    </Link>
                                {:else}
                                    <span class="px-3.5 py-2 text-xs font-bold text-slate-400 border border-slate-100 bg-slate-50/50 rounded-xl cursor-default select-none">
                                        {@html link.label}
                                    </span>
                                {/if}
                            {/each}
