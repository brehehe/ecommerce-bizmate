<script lang="ts">
    ...
    function submitTrackingNumber() {
        ...
        router.post(
            `/returns/${selectedReturn.id}/tracking`,
            {
            ...
            },
            {
                onSuccess: (pageObj) => {
                    ...
                    const updated = (pageObj.props as any).returns.data.find(
                        (r: any) => r.id === selectedReturn.id,
                    );
                    if (updated) {
...
<StorefrontLayout {storeName} {storeLogo} hideMobileFooter={true}>
    ...
    {#if selectedReturn}
        ...
        >
            ...
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <div
                class="relative z-10 bg-white w-full lg:max-w-2xl rounded-t-3xl lg:rounded-3xl p-6 max-h-[92dvh] overflow-y-auto shadow-2xl flex flex-col gap-6"
            ...
            >
                ...
                >
                    <div>
                        ...
                        <p class="text-xs text-slate-400 font-bold mt-1">
                            #{selectedReturn.return_number} &bull; Diajukan {fmtDate(
                ...
                {#if selectedReturn.status === 'ditolak'}
                    ...
                    >
                        ...
                        <div>
                            ...
                            >
                                Alasan penolakan: {selectedReturn.notes_admin ||
                ...
                {:else}
                    <div
                        class="bg-slate-50 rounded-2xl p-4 border border-slate-150/40"
                    >
                        <h4
                            class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 text-center"
                        >
                            Status Pengajuan
                        </h4>
                        ...
                        <div class="relative py-4">
                            <!-- Connecting Line -->
                            <div
                                class="absolute top-[28px] left-[8%] right-[8%] h-0.5 bg-slate-200 z-0"
                            >
                                <div
                            ...
                            <!-- Timeline Steps -->
                            <div
                                class="grid grid-cols-6 gap-1 text-center text-[9px] sm:text-[10px] font-bold text-slate-400 select-none relative z-10"
                            >
                                {#each [{ label: 'Diajukan', status: 'menunggu_review', icon: 'ti-file-text' }, { label: 'Disetujui', status: 'disetujui', icon: 'ti-circle-check' }, { label: 'Dikirim', status: 'barang_dikirim_customer', icon: 'ti-truck' }, { label: 'Diterima', status: 'barang_diterima_toko', icon: 'ti-building-warehouse' }, { label: 'Diproses', status: 'refund_diproses', icon: 'ti-refresh' }, { label: 'Selesai', status: 'selesai', icon: 'ti-circle-check-filled' }] as step, index}
                                    {@const isDone =
                ...
                <div class="space-y-3">
                    ...
                    >
                        {#each selectedReturn.items ?? [] as item}
                            <div
                                class="w-full flex items-center gap-3 py-2.5 first:pt-1 last:pb-1"
                            >
                                <div
                                    class="w-12 h-12 rounded-xl bg-slate-100 shrink-0 flex items-center justify-center border border-slate-200"
                                >
                                    <i
                                        class="ti ti-package text-slate-400 text-xl"
                                    ></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p
                                    ...
                                    {#if item.variant_name}
                                        <span
                                            class="inline-block text-[9px] font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 mt-1"
                                            >{item.variant_name}</span
                                        >
                                    {/if}
                                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div
                            class="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-xs text-slate-700 leading-relaxed font-semibold"
                        >
                                ...
                                {#each selectedReturn.media as mediaFile, idx}
                                    ...
                                    >
                                        {#if mediaFile.file_type === 'video'}
                                            ...
                                            >
                                                <i
                                                    class="ti ti-player-play-filled text-white text-lg"
                                                ></i>
                                            </div>
                                        {:else}
                                            <img
                ...
                {#if selectedReturn.type === 'refund'}
                    <div
                        class="bg-blue-50/50 rounded-2xl p-4 border border-blue-100 space-y-2"
                    >
                        <h4
                            class="text-xs font-black text-blue-900 uppercase tracking-wider flex items-center gap-1.5"
                        >
                            <i class="ti ti-credit-card"></i> Informasi Pengembalian
                            Dana (Refund)
                        </h4>
                        <div class="grid grid-cols-2 gap-y-2 text-xs">
                            <div>
                                <span class="text-blue-600/70 font-bold"
                                    >Total Refund</span
                                >
                                <p
                                    class="font-extrabold text-sm text-blue-900 mt-0.5"
                                >
                                    {fmt(selectedReturn.refund_amount)}
                                </p>
                            </div>
                            <div>
                                <span class="text-blue-600/70 font-bold"
                                ...
                                <p class="font-bold text-blue-900 mt-0.5">
                                    Transfer Bank Manual oleh Admin
                                </p>
                            </div>
                        </div>
                    </div>
                {:else}
                    ...
                    >
                        ...
                        <div
                            class="text-xs space-y-2 text-blue-900 leading-relaxed"
                        >
                            <div class="flex items-center gap-1.5">
                                <span class="font-bold text-blue-600/70"
                                    >Status Penukaran:</span
                                >
                                <span class="font-black">
                                    {#if selectedReturn.replacement_transaction_id}
                                        Barang Pengganti Telah Dikirim!
                                    {:else}
                                        Menunggu Barang Retur Tiba di Toko &amp;
                                        Diproses Admin
                                    {/if}
                                </span>
                            </div>
                            {#if selectedReturn.replacement_transaction_id}
                                <div
                                    class="grid grid-cols-2 gap-2 pt-1.5 border-t border-blue-100/50"
                                >
                                    <div>
                                        <span class="text-blue-600/70 font-bold"
                                            >Kurir</span
                                        >
                                        <p
                                            class="font-bold text-blue-900 mt-0.5"
                                        >
                                            {selectedReturn.replacement_courier_name ||
                                                '-'}
                                        </p>
                                    </div>
                                    <div>
                                        ...
                                        >
                                        <p
                                            class="font-mono font-bold text-blue-900 mt-0.5"
                                        >
                                            {selectedReturn.replacement_tracking_number ||
                                                '-'}
                                        </p>
                                    </div>
                                </div>
                                <div class="pt-1.5 flex justify-end">
                                    <Link
                                        href="/transactions/{selectedReturn.replacement_transaction_id}"
                                        class="inline-flex items-center gap-1 text-xs font-black hover:opacity-80 transition"
                                        style="color: {primary}"
                                    >
                ...
                {#if selectedReturn.status === 'disetujui'}
                    <div
                        class="bg-amber-50 border border-amber-100 rounded-2xl p-4 space-y-4"
                    >
                        <div class="flex gap-3 text-amber-800">
                            <i class="ti ti-info-circle text-xl shrink-0 mt-0.5"
                            ></i>
                            <div>
                                <p class="font-bold text-sm">
                                    Kirimkan Barang &amp; Input Resi
                                </p>
                                <p
                                    class="text-xs mt-1 text-amber-600 leading-relaxed font-semibold"
                                >
                                    Silakan kirimkan barang Anda kembali ke
                                    alamat toko kami. Setelah dikirim, silakan
                                    input nama kurir dan nomor resi pengiriman
                                    Anda di bawah ini agar kami dapat memantau
                                    dan memproses retur Anda segera.
                                </p>
                            </div>
                        </div>

                        <div
                            class="bg-white rounded-xl p-4 border border-amber-100/70 space-y-3"
                        >
                            <h5
                                class="text-xs font-black text-slate-700 uppercase tracking-wider"
                            >
                                Data Pengiriman Balik
                            </h5>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-slate-500 mb-1"
                                        for="courier-name">Nama Kurir</label
                                    >
                                    <input
                                        id="courier-name"
                                        type="text"
                                ...
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-slate-500 mb-1"
                                        for="tracking-number"
                                        >Nomor Resi <span class="text-red-500"
                                            >*</span
                                        ></label
                                    >
                                    <input
                        ...
                        <div class="grid grid-cols-2 gap-2 text-xs">
                            <div>
                                ...
                                <p class="font-bold text-slate-800 mt-0.5">
                                    {selectedReturn.return_courier_name || '-'}
                                </p>
                            </div>
                            <div>
                                <span class="text-slate-400 font-bold"
                                ...
                                >
                                <p
                                    class="font-mono font-bold text-slate-800 mt-0.5"
                                >
                ...
                <div class="flex gap-3 border-t border-slate-100 pt-4">
                    <Link
                        href="/transactions/{selectedReturn.transaction_id}"
