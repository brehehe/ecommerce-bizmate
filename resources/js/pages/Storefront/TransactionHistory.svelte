        ...
        <div class="max-w-6xl mx-auto px-4 py-6 pb-12">
            {#if statusCounts.all === 0}
                ...
                >
                    ...
                    >
                        <i class="ti ti-shopping-bag text-3xl text-slate-300"
                        ></i>
                    </div>
                    ...
                    <Link
                        ...
                        style="background:{primary}">Mulai Belanja</Link
                    >
                </div>
            {:else}
                <div class="max-w-6xl mx-auto space-y-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <div
                            class="bg-white rounded-2xl border border-slate-100 shadow-sm p-1.5 overflow-x-auto scrollbar-none flex gap-1.5 sticky top-[56px] lg:top-auto z-20"
                        >
                            {#each [{ key: 'all', label: 'Semua', count: statusCounts.all }, { key: 'belum_bayar', label: 'Belum Bayar', count: statusCounts.belum_bayar }, { key: 'berjalan', label: 'Dalam Proses', count: statusCounts.berjalan }, { key: 'selesai', label: 'Selesai', count: statusCounts.selesai }, { key: 'batal', label: 'Dibatalkan', count: statusCounts.batal }] as tab}
                                {@const isActive = selectedStatus === tab.key}
                                <button
                                    onclick={() => switchTab(tab.key)}
                                    class="px-4 py-2.5 rounded-xl text-xs font-bold transition whitespace-nowrap flex items-center gap-1.5 cursor-pointer flex-grow text-center justify-center {isActive
                                        ? 'text-white shadow-sm'
                                        : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}"
                                    style={isActive
                                        ? `background-color:${primary}; box-shadow: 0 4px 10px -2px ${primary}30;`
                                        : ''}
                                >
                                    {tab.label}
                                    {#if tab.count > 0}
                                        <span
                                            class="text-[10px] px-1.5 py-0.5 rounded-full font-black leading-none {isActive
                        ...
                        {#if filteredTransactions.length === 0}
                            <div
                                class="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center text-slate-400"
                            >
                                <i
                                    class="ti ti-clipboard-text text-4xl text-slate-200 mb-2 block"
                                ></i>
                            ...
                            <div class="space-y-4">
                                {#each filteredTransactions as trx (trx.id)}
                                    ...
                                    >
                                        <!-- Transaction Header -->
                                        <div
                                            class="px-4 py-3 flex items-center justify-between border-b border-slate-50 bg-slate-50/20"
                                        >
                                            <div>
                                                <p
                                        ...
                                        <!-- Items Preview -->
                                        <div
                                            class="px-4 py-3 divide-y divide-slate-50"
                                        >
                                            {#each (trx.items ?? []).slice(0, 2) as item}
                                                ...
                                                >
                                                    ...
                                                    {:else}
                                                        <div
                                                            class="w-12 h-12 rounded-xl bg-slate-50 shrink-0 flex items-center justify-center border border-slate-100"
                                                        >
                                                            <i
                                                                class="ti ti-package text-slate-300 text-lg"
                                                            ></i>
                                                        </div>
                                                    {/if}
                                                    <div class="flex-1 min-w-0">
                                                        ...
                                                        >
                                                            <p
                                                                class="text-xs font-bold text-slate-800 whitespace-pre-wrap break-words leading-tight group-hover:text-brand-blueRoyal transition"
                                                            >
                                                                {item.product_name}
                                                            </p>
                                                            {#if item.product?.is_digital}
                                                                <span
                                                                    class="text-[8px] font-black px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 border border-blue-100 uppercase tracking-wider shrink-0"
                                                                >
                                                                    Digital
                                                                </span>
                                                            {/if}
                                                        ...
                                                        {#if item.variant_name}
                                                            <span
                                                                class="inline-block text-[9px] font-bold px-1.5 py-0.5 bg-slate-50 border border-slate-100 rounded text-slate-500 mt-1"
                                            ...
                                            {#if (trx.items ?? []).length > 2}
                                                <p
                                                    class="text-xs text-slate-400 font-bold mt-2 pt-2 border-t border-slate-50"
                                                >
                                                    +{(trx.items ?? []).length -
                                                        2} produk lainnya
                                                </p>
                                            ...
                                            {#if trx.shipping_courier || trx.tracking_number || trx.booking_code}
                                                <div
                                                    class="pt-2 flex items-center justify-between text-xs gap-2"
                                                >
                                                    <div
                                                        class="flex items-center gap-1 text-slate-500"
                                                    >
                                                        <i
                                                            class="ti ti-truck text-slate-400"
                                                        ></i>
                                                        <span>
                                                            Kurir: <span
                                                                class="font-bold text-slate-700"
                                                                >{trx.courier_name ||
                                                                    trx.shipping_courier}</span
                                                            >
                                                            {#if trx.shipping_service}
                                                                <span
                                                                    class="text-slate-400"
                                                                    >({trx.shipping_service})</span
                                                                >
                                                            {/if}
                                                        </span>
                                                    </div>
                                                    ...
                                                    >
                                                        {#if trx.tracking_number}
                                                            <span
                                                                class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-100"
                                                            >
                                                                Resi: {trx.tracking_number}
                                                            </span>
                                                        {:else if trx.booking_code}
                                                            <span
                                                                class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-slate-50 text-slate-500 border border-slate-200"
                                                            >
                                        ...
                                        <!-- Footer -->
                                        <div
                                            class="px-4 py-3 border-t border-slate-50 bg-slate-50/20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
                                        >
                                            <div class="flex flex-col">
                                                ...
                                                >
                                                    ...
                                                    {#if trx.coins_redeemed > 0}
                                                        <span
                                                            class="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[9px] font-black bg-amber-50 text-amber-600 border border-amber-200/50"
                                                        >
                                                            <i
                                                                class="ti ti-coins"
                                                            ></i>
                                            ...
                                            >
                                                <button
                                                    onclick={(e) => {
                                                        ...
                                                        e.stopPropagation();
                                                        window.open(
                                                            `/transactions/${trx.id}/print-invoice?download=1`,
                                                            '_blank',
                                                        );
                                                    }}
                                                ...
                                                <div
                                                    class="flex-1 sm:flex-initial flex items-center justify-center gap-1 text-xs font-bold text-slate-500 bg-white border border-slate-200 px-3 py-2 sm:py-1.5 rounded-xl hover:bg-slate-55 hover:text-slate-700 hover:shadow-3xs transition"
                                                >
